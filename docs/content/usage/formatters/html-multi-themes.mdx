...
[CSS `light-dark()`](https://drafts.csswg.org/css-color-5/#light-dark) is a color function, so only `color` and `background-color` use it. `font-weight`, `font-style` and `text-decoration` are written as ordinary declarations holding the `light` theme's value, and every one of them either theme sets is also emitted as `--lumis-light-*` and `--lumis-dark-*` variables. Switching those three needs a rule of your own, see [Light/Dark Mode](/recipes/light-dark).
