...
Highlighting loads on demand, so preloading only decides *when* a language is
paid for. See [Languages](/languages).
...
parser size plus SHA-256 before writing.
...
To fill a directory the serving process will read — an image build, a prestart
step — use `lumis languages download`. It writes the same store and holds
nothing, and being a separate process is the point: failing there fails the
