...
const html: string = await highlight(
  "const x = 1",
...
// registered, including lazy entries
hl.registeredLanguages;

// loaded right now, usually just plaintext at first
hl.languages;
```
...
## Preload languages during application startup
...
process wants it. See [Languages](/languages).
...
Awaiting a preload before `startServer()` puts the CDN on the boot path, so an
...
`downloadLanguages()` is the caching half on its own — it writes the store and
holds nothing — for a build or prestart step preparing a directory the serving
process will read. Await that one; failing is the point. `lumis languages download`
