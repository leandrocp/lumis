...
Values are escaped, so an attribute can hold quotes and angle brackets.
...
let formatter = HtmlLinkedBuilder::new()
    ...
    .pre_class(Some("code-block".to_string()))
    .pre_attrs(vec![
        ("id".to_string(), "example".to_string()),
        ("data-panel".to_string(), "output".to_string()),
        ("class".to_string(), "overflow-x-auto".to_string()),
    ])
    .code_attrs(vec![("id".to_string(), "example-code".to_string())])
    .build()?;
```
