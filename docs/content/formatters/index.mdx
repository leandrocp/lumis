...
- JavaScript / TypeScript: pass a formatter object with `language` and `format(source)`, then call the sync free functions `highlightIter()` or `highlightEvents()` inside `format()` — see [source](https://github.com/leandrocp/lumis/blob/main/packages/javascript/lumis/src/formatter.ts)
