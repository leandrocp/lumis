...
description: Build your own output format on top of Lumis tokens in Rust, JavaScript, TypeScript, and Elixir.
keywords:
...
use lumis::{
    ...
    formatters::html::{
        closing_tags, open_code_tag, open_pre_tag,
        span_inline,
    },
...
const formatter: Formatter = {
  ...
  render(source, events) {
    ...
    parts.push(
      openPreTag({preClass: 'docs-demo', theme: frappe}),
    )
...
defmodule MinimalHtmlFormatter do
  ...
  def render(source, events, options) do
    ...
    # Resolving a scope against a theme is a table
    # lookup, so build the table once here rather
    # than crossing into Rust once per token.
    attrs =
      HTML.span_attrs(theme: theme, language: language)
    ...
    body =
      Enum.map(events, fn
        ...
        {:source, %{start: start, end: stop}} ->
          len = stop - start
          HTML.escape(binary_part(source, start, len))

        # Lumis adds event kinds as it grows. Render
        # the ones you know and skip the rest, or a
        # newer Lumis raises FunctionClauseError here.
        _event ->
          []
      end)
...
Lumis does that once at the end. When the result is not one string, such as a
list of rendered lines, `Lumis.highlight_events/3` returns the same event stream
without a formatter.

For line-based output, reach for `render_lines_from_events/3` rather than
splitting rendered markup on newlines. The results contain no line terminators;
join wrapped lines with `"\n"` when building a block. Use the
[shared line layout CSS](/formatters/html-line-migration#custom-css) for a custom
formatter's full-width lines. A `<span>` that crosses a newline has to
        ...
        {output, scopes, tables} ->
          ...
          case scopes do
            ...
            [{scope, language} | _rest] ->
              # One table per language, built once
              # and read per token.
              tables =
              ...
              style =
                ANSI.style_for(tables[language], scope)
...
Lumis.highlight!("defmodule App do\nend",
  formatter:
    {MinimalTerminalFormatter,
     language: "elixir", theme: "dracula"}
)
```
...
The built-in formatters take options that enrich highlighting. Custom
formatters take the same options, as a trailing argument to the token iterator.
The flat iterator presents rainbow brackets as the same six scopes the built-ins
render for backward compatibility.

<Tabs groupId="runtime" persist items={['JavaScript / TypeScript', 'Rust']}>
...
highlightIter(
  ...
  this.language,
  frappe,
  (text, language, _range, scope) => {
    // scope is punctuation.bracket.rainbow.1
    // through .6 on bracket pairs
  },
...
```rust
use lumis::highlight::{
    highlight_iter_with_options, HighlightOptions,
};
...
const options = {rainbowBrackets: true}
const events = highlightEvents(source, rust, options)

for (const event of events) {
  if (event.type === 'start') {
    console.log(event.scope, event.language)
  }
  if (
    event.type === 'decorationStart' &&
    event.decoration.type === 'rainbowBracket'
  ) {
    console.log(
      'real zero-based depth',
...
use lumis::highlight::{
    highlight_events_with_options, HighlightOptions,
};
...
for event in events {
    ...
    if let HighlightEvent::DecorationStart {
        decoration: Decoration::RainbowBracket { depth },
    } = &event
    {
        println!("real zero-based depth: {depth}");
    }
}
...
name the npm package's event carries directly. Rainbow brackets are decoration
events instead: their `depth` does not wrap, leaving a custom formatter free to
use more than six colors or a non-color representation.
...
{:ok, events} =
  Lumis.highlight_events(source, "rust",
...
for event <- events do
  case event do
    {:start, %{scope: scope, language: language}} ->
      IO.inspect({scope, language})

    {:decoration_start, %RainbowBracket{depth: depth}} ->
      IO.puts("real zero-based depth: #{depth}")
...
The events are the ones a `Lumis.Formatter` receives, so every
`Lumis.Formatter.HTML` helper applies. `render_lines_from_events/3` turns them
into one HTML fragment per source line.
...
host's convention — `scope_to_class` in Rust and Elixir, `scopeToClass` in
the npm package — and so do argument shapes.

| Runtime | Module | Reference |
| --- | --- | --- |
| JavaScript / TypeScript | `@lumis-sh/lumis/formatters/html` | [source](https://github.com/leandrocp/lumis/blob/main/packages/javascript/lumis/src/formatter/html.ts) |
| JavaScript / TypeScript | `@lumis-sh/lumis/formatters/ansi` | [source](https://github.com/leandrocp/lumis/blob/main/packages/javascript/lumis/src/formatter/ansi.ts) |
...
| Take the block attributes instead of the rendered tag, to merge your own | `pre_attrs`, `multi_themes_pre_attrs`, `code_attrs` |
| Render a tag from attributes, escaping the values and checking the names | `open_tag`, `valid_attr_name` |
...
syntax spans. The unterminated final line carries none, so pass each result to
`wrap_line` directly in every runtime.
...
read a constant costs more than the highlighting; the npm package exports
