...
`Lumis.Languages.download/2` and the JavaScript / TypeScript `downloadLanguages()` accept the same
...
The CLI has no lock. `lumis highlight` resolves freely and `lumis languages
cache` fills its store — a viewer is not the application whose output a lock
governs. Pinning belongs to a project and its own runtime; see
[Pinning parsers](/advanced/lock).
