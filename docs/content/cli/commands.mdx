...
`@lumis-sh/wasm-bundle-*` package. `Lumis.Languages.async_load/1` and the
JavaScript / TypeScript `loadLanguages()` accept the same names in application
code.
...
download` fills its store — a viewer is not the application whose output a lock
