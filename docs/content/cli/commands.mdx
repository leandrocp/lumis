...
| `lumis languages add` | Record languages in `lumis-lock.toml` and cache them |
| `lumis languages remove` | Stop recording languages in `lumis-lock.toml` |
| `lumis languages update` | Re-resolve locked packages and move `lumis-lock.toml` |
| `lumis languages install` | Cache exactly what `lumis-lock.toml` names |
...
| `--rainbow-brackets` | | Render matched brackets by nesting depth |
...
Accepted by every formatter except `bbcode-scoped`, which has nothing to render a
number into:

| Flag | Short | Description |
| --- | --- | --- |
| `--line-numbers` | `-n` | Render a line number gutter |
...
| `--pre-attr <name[=value]>` | | Attribute for the wrapping `<pre>` tag; a bare name writes the boolean form. Repeat for more than one |
| `--no-pre-attr <name>` | | Attribute to leave off the wrapping `<pre>` tag |
| `--code-attr <name[=value]>` | | Attribute for the nested `<code>` tag; a bare name writes the boolean form. Repeat for more than one |
| `--no-code-attr <name>` | | Attribute to leave off the nested `<code>` tag, such as `tabindex` |
...
`Lumis.Languages.cache/2` and the JavaScript / TypeScript `cacheLanguages()` accept the same
...
Without a lock, whichever compatible version a machine downloads first is the
one it keeps, so two machines can render the same document differently and
nothing records which parser produced which output.

The lock is for runtimes that fetch parsers at runtime: Elixir, the CLI, and any
future binding. A Rust project already declares its languages as `Cargo.toml`
features, and a JavaScript project that installs `@lumis-sh/wasm-*` already
declares them in `package.json` — neither needs a lock, and no project should
have two declarations.

`lumis highlight` and `lumis dump` **never read a project's lock.** They resolve
freely against the CLI's own store, because a viewer is not the application
whose output a lock governs. `lumis languages install` is what consumes one.

`lumis-lock.toml` pins an exact version and its digests. Commit it, and every
machine and CI run resolves the same parsers.

```bash title="Record a language"
lumis languages add rust
```

The lock is created at the repository root the first time, whichever directory
you run from:
...
`manifest_sha256` covers the package metadata itself, not only the parser it
describes, so a registry cannot serve different bytes than the ones recorded.

Commit `lumis-lock.toml`, and ignore the advisory sidecar beside it:

```gitignore title=".gitignore"
.lumis-lock.toml.lock
```

The sidecar is how two `lumis languages add` runs avoid overwriting each other's
entries. It stays on disk deliberately — deleting it would race a process that
has already opened it and is waiting.

Rows are keyed on the package because one package can back several languages —
`ejs` and `erb` are both `@lumis-sh/wasm-embedded-template`. `languages` lists
what you added, so a lock can pin `ejs` without also allowing `erb`.

```bash title="Materialize the lock, resolving nothing"
lumis languages install
```

Use this in CI and in a Dockerfile, the way you would `npm ci`. It downloads
exactly what the lock names and fails if the store holds a different version,
so a stale machine cache cannot quietly win.

```bash title="Move a version deliberately"
lumis languages update rust
lumis languages update --all
```

`update` is the only command that changes a pinned version. It resolves
everything before writing anything, so a registry that is down for one package
leaves the lock untouched rather than moving the rest.

```bash title="Stop pinning a language"
lumis languages remove rust
```

A bundle is shorthand when you add it, and its members are what get recorded.
Re-run `lumis languages add bundle-web` to pick up a language the bundle gained,
so widening what a project allows stays a visible diff.
