...
The CLI does not read or write a project's lock. It is its own runtime with its
own store, so `lumis highlight` resolves freely and `lumis languages cache`
fills the store — a viewer is not the application whose output a lock governs,
and a highlighter whose behaviour changed with the working directory would be
worse than one that does not.

A lock belongs to a project, and the runtime that owns that project manages it:
`mix lumis.*` in Elixir, `package.json` in JavaScript, `Cargo.toml` features in
Rust. See [Architecture](/architecture) for which runtimes need one and why.
