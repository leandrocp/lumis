...
- `parser-sizes.json` is measured off what is published, so it lags a parser
  release by design. Run `mise run wasm-sizes {name}` after one lands to refresh
  the `Size` and `Memory` columns in the language catalog; a whole-series move
  wants the unfiltered run.
