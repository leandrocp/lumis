...
impl Highlighter {
    ...
    pub fn set_match_limit(&mut self, match_limit: u32) -> Result<(), Error> {
        ...
        if match_limit < self.match_limit {
            // A cursor's capture list pool never shrinks, and acquiring a list
            // takes an already-grown one before it consults the limit, so a
            // lower bound only binds on a cursor that starts empty. Highlighters
            // are reused, so without this a worker keeps whatever bound its
            // first document asked for.
            self.cursors.clear();
        }
